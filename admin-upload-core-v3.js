 ...
 * Extração Universal de ID: Resolve document, video, animation ou photo.
 */
...
console.log("%c[VITTALIX-HD] MOTOR CORE V3 (UNIVERSAL RESOLVER) ATIVO.", "color: #D4AF37; font-weight: bold;");
...
async function vittalixUpload(file, onProgressCallback = null) {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        if (showModal) updateStatusMsg(totalChunks > 1 ? `Transmitindo parte ${i+1} de ${totalChunks}...` : `Otimizando obra de arte...`);
        ...
        try {
            ...
            // RESOLVEDOR UNIVERSAL: Acha o ID em qualquer gaveta
            ...
            if (!mediaObj || !mediaObj.file_id) {
                console.error("[VITTALIX-HD] Resposta do Telegram inesperada:", resObj);
                throw new Error("Falha na extração do ID de mídia.");
            }

            fileIds[i] = mediaObj.file_id;
            
            // Capturar miniatura nativa de qualquer objeto que a possua
            if (i === 0) {
                if (isImage) {
                    nativeThumbId = fileIds[i];
                } else if (mediaObj.thumb) {
                    nativeThumbId = mediaObj.thumb.file_id;
                }
            }
...
function xhrUpload(url, formData, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
            ...
            catch (e) { reject(new Error("Erro ao processar resposta do servidor.")); }
        };
        ...
        modal.innerHTML = `
            <div style="position: fixed; inset: 0; background: rgba(0,0,0,0.95); z-index: 99999; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(25px);">
                <div style="text-align: center; color: #D4AF37; font-family: 'Bodoni Moda', serif; width: 100%; max-width: 400px;">
                    ...
                    <h2 style="font-size: 1.2rem; margin-bottom: 1.5rem; letter-spacing: 2px;">VITTALIX-HD CORE V3</h2>
